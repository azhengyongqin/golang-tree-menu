...
// FindFatherNode 在 allTree 中查询 nodes 中节点的所有父节点
// nodes 要查询父节点的子节点数组
// allTree 所有节点数组
func FindFatherNode(nodes, allNodes []INode) (respNodes []INode) {
	nodeMap := make(map[int]INode)
	for _, v := range nodes {
		recursiveFindFatherNode(nodeMap, allNodes, v)
	}
...
// recursiveFindFatherNode 递归查询父节点
func recursiveFindFatherNode(nodeMap map[int]INode, allNodes []INode, node INode) {
